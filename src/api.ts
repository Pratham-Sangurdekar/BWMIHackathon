...
import type { BookingAttempt, JourneyResult, Scenario, SearchQuery, TxState, User } from "./types.js";
...
const event = (label: string) => ({ time: now(), label });
...
export function validateBookingDate(date: string): string | undefined {
  if (!date) return "Please select a journey date.";
  ...
  if (selected < min) return "Journey date cannot be in the past.";
  if (selected > max) return "Bookings are available for this month and the next two months.";
  return undefined;
}
...
export async function login(email: string, password: string): Promise<User> {
  ...
  if (!user) throw new Error("Email address is not registered.");
  if (user.password !== password) throw new Error("Password is incorrect.");
  return { email: user.email, name: user.name };
}

export async function search(query: SearchQuery): Promise<JourneyResult[]> {
  await sleep(500);
  const dateError = validateBookingDate(query.date);
  if (dateError) throw new Error(dateError);
  if (!query.from || !query.to) throw new Error("Please choose both FROM and TO stations.");
  if (query.from === query.to) throw new Error("FROM and TO stations cannot be the same.");
  if (query.disability && !query.disabilityId?.trim()) throw new Error("Please enter the mock disability concession ID.");
  if (query.railPass && !query.passNumber?.trim()) throw new Error("Please enter the mock railway pass number.");
...
export function stationsFor(term: string) {
  return searchStations(term);
}

export function bookingMeaning(state: TxState) {
  const table: Record<TxState, { citizenState: string; money: string; ticket: string; next: string; retryAllowed: boolean }> = {
...
function saveAttempt(attempt: BookingAttempt) {
...
export function duplicateAttempt(journey: JourneyResult, query: SearchQuery): BookingAttempt | undefined {
...
export async function startBooking(journey: JourneyResult, query: SearchQuery): Promise<BookingAttempt> {
  ...
  saveAttempt(attempt);
  await sleep(350);
  attempt.state = "PAYMENT_PENDING"; attempt.timeline.push(event("Payment confirmation started")); saveAttempt(attempt);
  await sleep(550);
  if (query.scenario === "paymentFailed") {
    attempt.state = "PAYMENT_FAILED"; attempt.retryAllowed = true; attempt.timeline.push(event("Payment failed")); saveAttempt(attempt); return attempt;
  }
  attempt.state = "PAYMENT_SUCCESS"; attempt.timeline.push(event("Payment received")); saveAttempt(attempt);
  await sleep(450);
  ...
  saveAttempt(attempt);
  if (query.scenario === "normal") return finalizeBooked(attempt);
  return attempt;
}

function finalizeBooked(attempt: BookingAttempt): BookingAttempt {
...
export async function checkBookingStatus(id: string): Promise<BookingAttempt> {
  await sleep(650);
  const attempt = getState().attempts.find((item) => item.id === id);
  if (!attempt) throw new Error("We could not find this booking attempt on this device.");
  if (attempt.state !== "BOOKING_UNKNOWN" && attempt.state !== "BOOKING_PENDING" && attempt.state !== "NOT_BOOKED" && attempt.state !== "REFUND_PENDING") return attempt;
  if (attempt.query.scenario === "unknownNotBooked") {
    if (attempt.state === "REFUND_PENDING") {
      attempt.state = "REFUNDED"; attempt.timeline.push(event("Refund completed")); saveAttempt(attempt); return attempt;
    }
    attempt.state = "NOT_BOOKED"; attempt.retryAllowed = true; attempt.timeline.push(event("No ticket was created"));
    attempt.state = "REFUND_PENDING"; attempt.refundId = `RF-${Date.now().toString(36).toUpperCase()}`; attempt.timeline.push(event("Refund started"));
    saveAttempt(attempt);
    return attempt;
  }
...
export function stationLabel(code: string) {
